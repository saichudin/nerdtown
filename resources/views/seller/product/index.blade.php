@extends('front-base')

@section('content')
    <!-- Page Content -->
    <div class="container pt-4">
        <h1>List Product</h1>

        <div class="">
            <a href="{{ route('seller.product.create') }}" class="btn btn-sm btn-outline-success float-right">
                <i class="zmdi zmdi-plus"></i>
                {{ __('Add New Product') }}
            </a>
        </div>
        <br/><br/>

        <div class="card-block">
            <table class="table table-striped table-hover">
                <thead>
                <tr>
                    <th>&nbsp;</th>
                    <th>{{ __('Name') }}</th>
                    <th>{{ __('Sales') }}</th>
                    {{--<th>{{ __('Categorization') }}</th>--}}
                    <th>{{ __('State') }}</th>
                    <th style="width: 10%">&nbsp;</th>
                </tr>
                </thead>

                <tbody>
                <?php $defaultThumbnail = 'data:image/png;base64,'; ?>
                @foreach($products as $product)
                    <tr>
                        <td>
                            <img src="{{ $product->getThumbnailUrl() ?: $defaultThumbnail }}" class="mw-100" style="height: 2.5em;" />
                        </td>
                        <td>
                            <span class="font-lg mb-3 font-weight-bold">
                                <a href="#">
                                    {{ $product->name }}
                                </a>
                            </span>
                            <div class="text-muted" title="{{ __('SKU') }}">{{ $product->sku }}</div>
                        </td>
                        <td>
                            <span class="mb-3">
                                {{ $product->units_sold }} {{ __('units sold') }}
                            </span>
                            <div class="text-muted" title="{{ $product->last_sale_at ? $product->last_sale_at->format(__('Y-m-d H:i')) : '' }}">
                                {{ __('Last sale') }}
                                {{ $product->last_sale_at ? $product->last_sale_at->diffForHumans() : __('never') }}
                            </div>
                        </td>
                        {{--<td>--}}
                            {{--<div class="mt-2">--}}
                                {{--@foreach($product->taxons as $taxon)--}}
                                    {{--<span class="badge badge-pill badge-secondary">--}}
                                        {{--{{ $taxon->name }}--}}
                                    {{--</span>--}}
                                {{--@endforeach--}}
                            {{--</div>--}}
                        {{--</td>--}}
                        <td>
                            <div class="mt-2">
                                <span class="badge badge-pill badge-{{$product->is_active ? 'success' : 'secondary'}}">{{ $product->state->label() }}</span>
                            </div>
                        </td>
                        <td>
                                <a href="{{ route('seller.product.edit', ['product' => $product]) }}"
                                   class="btn btn-xs btn-outline-primary btn-show-on-tr-hover float-right">{{ __('Edit') }}
                                </a>
                                {!! Form::open(['route' => ['seller.product.destroy', $product],
                                        'method' => 'DELETE',
                                        'onsubmit' => 'return confirm("are you sure delete ?")'
                                    ])
                                !!}
                                <button class="btn btn-xs btn-outline-danger btn-show-on-tr-hover float-right">{{ __('Delete') }}</button>
                                {!! Form::close() !!}
                        </td>
                    </tr>
                @endforeach
                </tbody>

            </table>

            @if($products->hasPages())
                <hr>
                <nav>
                    {{ $products->links() }}
                </nav>
            @endif

        </div>
    </div>
    <!-- /.container -->
@endsection
